...
────────────────────────────────────────────────────────────────────────────────────────────────────────────
                       ...
                       {Fore.CYAN}BlastyOP v{SELFBOT.__version__} | {Fore.CYAN}Logged in as: {BlastyOP.user.name}#{BlastyOP.user.discriminator} {Fore.CYAN}| ID: {Fore.CYAN}{Arsky.user.id}   
                       {Fore.CYAN}Nitro Sniper | {Fore.CYAN}{nitro}
                       {Fore.CYAN}Cached Users: {Fore.CYAN}{len(BlastyOP.users)}
                       {Fore.CYAN}Guilds: {Fore.CYAN}{len(BlastyOP.guilds)}
                       {Fore.CYAN}Prefix: {Fore.CYAN}{BlastyOP.command_prefix}
    ''' + Fore.RESET)
...
def Init():
    token = config.get('token')
    try:
        BlastyOP.run(token, bot=False, reconnect=True)
...
colorama.init()
BlastyOP = discord.Client()
BlastyOP = commands.Bot(description='BlastyOP Selfbot', command_prefix=prefix, self_bot=True)

BlastyOP.antiraid = False
BlastyOP.msgsniper = True
BlastyOP.slotbot_sniper = True
BlastyOP.giveaway_sniper = True
BlastyOP.mee6 = False
BlastyOP.mee6_channel = None
BlastyOP.yui_kiss_user = None
BlastyOP.yui_kiss_channel = None
BlastyOP.yui_hug_user = None
BlastyOP.yui_hug_channel = None
BlastyOP.sniped_message_dict = {}
BlastyOP.sniped_edited_message_dict = {}
BlastyOP.whitelisted_users = {}
BlastyOP.copycat = None
BlastyOP.remove_command('help')
...
async def on_message_edit(before, after):
    await BlastyOP.process_commands(after)


@BlastyOP.event
async def on_message(message):
    if BlastyOP.copycat is not None and BlastyOP.copycat.id == message.author.id:
    ...
    await BlastyOP.process_commands(message)
...
async def on_member_ban(guild: discord.Guild, user: discord.user):
    if BlastyOP.antiraid is True:
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in BlastyOP.whitelisted_users.keys() and i.user.id in BlastyOP.whitelisted_users[
        ...
        try:
            ...
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.kick):
                if guild.id in BlastyOP.whitelisted_users.keys() and i.user.id in BlastyOP.whitelisted_users[
                    ...
                    print('banned')
                    await guild.ban(i.user, reason="BlastyOP Anti-Nuke")
        except Exception as e:
            print(e)


@BlastyOP.command(aliases=["queue"])
...
@BlastyOP.command(aliases=["lyric"])
async def lyrics(ctx, *, args):
    await ctx.message.delete()
    await ctx.send("Showing lyrics for " + args)


@BlastyOP.command(aliases=[])
...
@BlastyOP.command(aliases=['ar', 'antiraid'])
...
@BlastyOP.command(aliases=['wl'])
async def whitelist(ctx, user: discord.Member = None):
    await ctx.message.delete()
    if user is None:
        await ctx.send("Please specify a user to whitelist")
    else:
        if ctx.guild.id not in BlastyOP.whitelisted_users.keys():
            BlastyOP.whitelisted_users[ctx.guild.id] = {}
    ...
    # else:
    #     user = BlastyOP.get_user(id)
    #     if user is None:
    #         await ctx.send("Couldn't find that user")
    #         return
    #     if ctx.guild.id not in Arsky.whitelisted_users.keys():
    #         BlastyOP.whitelisted_users[ctx.guild.id] = {}
    #     if user.id in BlastyOP.whitelisted_users[ctx.guild.id]:
    #         await ctx.send('That user is already whitelisted')
    #     else:
    #         BlastyOP.whitelisted_users[ctx.guild.id][user.id] = 0
    #         await ctx.send("Whitelisted **" + user.name.replace("*", "\*").replace("`", "\`").replace("_","\_") + "#" + user.discriminator + "**")


@BlastyOP.command(aliases=['wld'])
async def whitelisted(ctx, g=None):
    await ctx.message.delete()
    if g == '-g' or g == '-global':
        whitelist = '`All Whitelisted Users:`\n'
        for key in BlastyOP.whitelisted_users:
            for key2 in BlastyOP.whitelisted_users[key]:
                user = BlastyOP.get_user(key2)
        ...
        for key in Arsky.whitelisted_users:
            if key == ctx.guild.id:
                for key2 in BlastyOP.whitelisted_users[ctx.guild.id]:
                    user = BlastyOP.get_user(key2)
                    whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
...
@BlastyOP.command(aliases=['clearwl', 'clearwld'])
async def clearwhitelist(ctx):
    await ctx.message.delete()
    BlastyOP.whitelisted_users.clear()
...
async def yuikiss(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        BlastyOP.yui_kiss_user = user.id
        BlastyOP.yui_kiss_channel = ctx.channel.id
        if BlastyOP.yui_kiss_user is None or BlastyOP.yui_kiss_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while BlastyOP.yui_kiss_user is not None and BlastyOP.yui_kiss_channel is not None:
            await BlastyOP.get_channel(BlastyOP.yui_kiss_channel).send('yui kiss ' + str(BlastyOP.yui_kiss_user), delete_after=0.1)
...
async def yuihug(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        BlastyOP.yui_hug_user = user.id
        BlastyOP.yui_hug_channel = ctx.channel.id
        if BlastyOP.yui_hug_user is None or BlastyOP.yui_hug_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
...
@BlastyOP.command(aliases=["automee6"])
async def mee6(ctx, param=None):
    ...
    while BlastyOP.mee6 is True:
        ...
        await BlastyOP.get_channel(BlastyOP.mee6_channel).send(random.choice(sentences), delete_after=0.1)
        await asyncio.sleep(60)


@BlastyOP.command(aliases=['slotsniper', "slotbotsniper"])
async def slotbot(ctx, param=None):
    await ctx.message.delete()
    BlastyOP.slotbot_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        BlastyOP.slotbot_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        BlastyOP.slotbot_sniper = False
...
@BlastyOP.command(aliases=['giveawaysniper'])
async def giveaway(ctx, param=None):
    await ctx.message.delete()
    BlastyOP.giveaway_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        BlastyOP.giveaway_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        BlastyOP.giveaway_sniper = False
...
async def on_message_delete(message):
    if message.author.id == BlastyOP.user.id:
        return
    if BlastyOP.msgsniper:
        if isinstance(message.channel, discord.DMChannel) or isinstance(message.channel, discord.GroupChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    message.content) + "\n\n**Attachments:**\n" + links
                await message.channel.send(message_content)
    if len(BlastyOP.sniped_message_dict) > 1000:
        BlastyOP.sniped_message_dict.clear()
    attachments = message.attachments
    if len(attachments) == 0:
        channel_id = message.channel.id
        message_content = "`" + str(discord.utils.escape_markdown(str(message.author))) + "`: " + str(
            message.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        BlastyOP.sniped_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
        ...
        if isinstance(before.channel, discord.DMChannel) or isinstance(before.channel, discord.GroupChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    before.content) + "\n\n**Attachments:**\n" + links
                await before.channel.send(message_content)
    if len(BlastyOP.sniped_edited_message_dict) > 1000:
        BlastyOP.sniped_edited_message_dict.clear()
    attachments = before.attachments
    if len(attachments) == 0:
        ...
        message_content = "`" + str(discord.utils.escape_markdown(str(before.author))) + "`: \n**BEFORE**\n" + str(
            before.content).replace("@everyone", "@\u200beveryone").replace("@here",
                                                                            "@\u200bhere") + "\n**AFTER**\n" + str(
            after.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        BlastyOP.sniped_edited_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
async def snipe(ctx):
    ...
    if currentChannel in BlastyOP.sniped_message_dict:
        await ctx.send(BlastyOP.sniped_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")


@BlastyOP.command(aliases=["esnipe"])
async def editsnipe(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in BlastyOP.sniped_edited_message_dict:
        await ctx.send(BlastyOP.sniped_edited_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_thumbnail(url=BlastyOP.user.avatar_url)
    ...
    elif str(category).lower() == "BlastyOP":
...
@BlastyOP.command(aliases=["img", "searchimg", "searchimage", "imagesearch", "imgsearch"])
...
@BlastyOP.command(aliases=["addemoji", "stealemote", "addemote"])
...
@BlastyOP.command(aliases=["stopcopycatuser", "stopcopyuser", "stopcopy"])
async def stopcopycat(ctx):
    await ctx.message.delete()
    if BlastyOP.user is None:
...
@BlastyOP.command(aliases=["copycatuser", "copyuser"])
async def copycat(ctx, user: discord.User):
    await ctx.message.delete()
    BlastyOP.copycat = user
    await ctx.send("Now copying " + str(Arsky.copycat))


@BlastyOP.command(aliases=["9/11", "911", "terrorist"])
...
@BlastyOP.command(aliases=["jerkoff", "ejaculate", "orgasm"])
async def cum(ctx):
    await ctx.message.delete()
...
@BlastyOP.command(aliases=["spamchangegcname", "changegcname"])
async def spamgcname(ctx):
    await ctx.message.delete()
    if isinstance(ctx.message.channel, discord.GroupChannel):
        watermark = "BlastyOP❥GOD"
        name = ""
...
@BlastyOP.command(aliases=["fakename"])
...
@BlastyOP.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
...
@BlastyOP.command(aliases=["distort"])
...
@BlastyOP.command(aliases=['markasread', 'ack'])
async def read(ctx):
    await ctx.message.delete()
...
@BlastyOP.command(aliases=["deepfry"])
...
@BlastyOP.command(aliases=["pixel"])
